...
class modelRNN(nn.Module):
    def __init__(self,input_size, hidden_size, num_class):
        ...
        self.emb = nn.Embedding.from_pretrained(word2vec.vectors) # word2vec.vectors property to represent a singular resume sample as an array of size (word_count, embedding_size)
...
def train(model, train, val, n_epochs = 5, lr=1e-5):
    ...
    for epoch in range(n_epochs):
        for resume, label in train: #training data will contain resume content paired with predefined job category label
            optimizer.zero_grad()
